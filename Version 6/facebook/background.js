// extensions/facebook/background.js
// Facebook-specific background script.
// Adds the mobile UA spoofing handler for Facebook video extraction.
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'fetchMobileFacebookVideo') {
        ...
        .then(r => r.text())
        .then(html => {
            // Try multiple patterns to find progressive MP4 URLs in the mobile HTML
            let videoUrl = null;

            // Pattern 1: Direct src attribute with .mp4
            const srcMatch = html.match(/src="([^"]+?\.mp4[^"]*?)"/);
            if (srcMatch && srcMatch[1]) {
                videoUrl = srcMatch[1].replace(/&amp;/g, '&').replace(/\\\//g, '/');
            }

            // Pattern 2: JSON-encoded playable_url in inline scripts
            if (!videoUrl) {
                const playableMatch = html.match(/"playable_url"\s*:\s*"([^"\\]*(?:\\.[^"\\]*)*)"/);
                if (playableMatch && playableMatch[1]) {
                    try {
                        videoUrl = JSON.parse('"' + playableMatch[1] + '"');
                    } catch (e) {}
                }
            }

            // Pattern 3: HD playable URL
            if (!videoUrl) {
